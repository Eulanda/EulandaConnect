...
Describe 'Test-FtpFolder' -Tag 'integration', 'ftp' {
    InModuleScope 'EulandaConnect' {
        ...
        It "Verifies if the FTP file 'License.md' exists at the beginning" {
            $result = Test-FtpFile -server $server -user $user -password $secure -remoteFile 'License.md'
            $result | Should -Be $true
        }

        It "Verifies if the FTP file 'License.txt' does not exists at beginning" {
        ...
        It "Verifies if renamed FTP file 'License.txt' exists" {
            $result = Test-FtpFile -server $server -user $user -password $secure -remoteFile 'License.txt'
            $result | Should -Be $true
        }
        ...
        It "Verifies if the FTP file 'License.md' exists like at beginning" {
            $result = Test-FtpFile -server $server -user $user -password $secure -remoteFile 'License.md'
            $result | Should -Be $true
        }
